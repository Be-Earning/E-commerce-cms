...
import { ICustomerInfor, ICustomerTable } from '~/@types/models'
...
const ListCustomer = memo(() => {
  ...
  const listCustomersByRole = useMemo(() => {
    return listCustomers.filter((customer) => +customer.role === Role.CUSTOMER)
  }, [listCustomers])

  const listDataTable: ICustomerTable[] = useMemo(() => {
  ...
  const columsMangement: CustomGridColDef[] = useMemo(() => {
    const allColumns: CustomGridColDef[] = [
      ...
      },
      {
        ...
        width: smDown ? 140 : mdDown ? 160 : lgDown ? 200 : xlDown ? 300 : 350,
        filterable: false,
        sortable: false,
        disableColumnMenu: true,
        renderCell: ({ value }) => (
      ...
      },
      {
        ...
        width: smDown ? 100 : mdDown ? 120 : lgDown ? 160 : xlDown ? 250 : 320,
        filterable: false,
        sortable: false,
        disableColumnMenu: true,
        renderCell: ({ value }) => (
      ...
      },
      {
        ...
        disableColumnMenu: true,
        renderCell: ({ value }) => {
          const pathDetail = `/customer/detail/${value}`
          return (
  ...
  useEffect(() => {
    ...
    if (searchTerm)
      filtered = filtered.filter((customer) =>
        customer.fullName?.toLowerCase().includes(searchTerm.toLowerCase().trim())
      )
  ...
  return (
    <div className={'bg-list-customer bg-top bg-no-repeat xs:p-4 sm:p-5'}>
      <div className='bg-white/[.76] shadow-header backdrop-blur-[200px] xs:mt-14 xs:rounded-2xl xs:px-[14px] xs:py-6 sm:mt-[68px] sm:rounded-3xl sm:px-5 sm:py-5 md:px-10 md:py-10 lg:px-12 lg:py-10 xl:px-20 xl:py-12'>
        <FilterLayout title={`Customer Management`}>

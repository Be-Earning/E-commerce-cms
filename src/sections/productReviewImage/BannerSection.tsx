...
import { ProductCardBannerImage } from '~/components/productCardBannerImage'
...
const BannerSection = memo(({ purchases, trend, product }: BannerSectionProps) => {
  ...
  const { product: productInfo, variants, attributes } = product || {}
  ...
  const listColors = useMemo(
    () =>
      attributes?.[0].length > 0
        ? [...new Set(attributes.map((attrGroup) => attrGroup.find((attr) => attr.key === 'color')?.value))]
        : [],
    [attributes]
  ) as string[]

  const productQuantity = useMemo(
    () => variants.reduce((total: number, variant) => total + +variant.priceOptions.quantity, 0),
    [variants]
  )

  const [activeSlide, setActiveSlide] = useState<number>(0)
  const [showColors, setShowColors] = useState<boolean>(false)
  const [colorSelect, setColorSelect] = useState<string>(listColors?.[0])
    ...
    <>
      <section className='relative overflow-hidden border-b-[3px] border-solid border-[#FFFFFF52] bg-ln-gray xs:h-[844px] sm:h-[810px]'>
        ...
        <div className='relative xs:mt-[180px] sm:mt-10'>
          ...
          >
            {listImages?.map((img, index: number) => (
              <SwiperSlide key={`${img}-${index}`} style={{ overflow: 'unset' }}>
                <div className='relative mx-auto w-fit border border-solid'>
                  <img
                    src={img}
                    alt='product-banner'
                    className='mx-auto object-cover xs:h-[438px] xs:w-[380px] sm:h-[550px] md:h-[600px] lg:h-[700px]'
                  />
                  <div className='absolute -left-3 -top-3 z-20 flex size-6 shrink-0 items-center justify-center rounded-full bg-black-main text-[14px] text-white'>
                    {index + 2}
                  </div>
                </div>
              </SwiperSlide>
          ...
          <div className='flex items-center gap-3'>
            <p className='text-blackMain/[.64] font-customRegular xs:text-[10.24px] sm:text-[16px]/[16.8px]'>
              ...
              <span className='text-blackMain font-customMedium'>
                {formatDate(+productInfo.params.expiryTime, 'h:mm:ss')}
              </span>
            </p>
            ...
            <p className='text-blackMain/[.64] font-customRegular xs:text-[10.24px] sm:text-[16px]/[16.8px]'>
              Left: <span className='text-blackMain font-customMedium'>{productQuantity} items</span>
            </p>
          </div>
        </div>
        ...
        <div className='absolute flex items-center gap-4 xs:bottom-6 xs:right-[4%] sm:bottom-[35px] sm:right-[118px]'>
          ...
          <Link to={productInfo.params.videoUrl}>
        ...
        <div className='absolute z-50 xs:bottom-[250px] xs:right-4 sm:bottom-[118px] sm:right-[177px]'>
          <ProductCardBannerImage product={product} />
        </div>

        {productInfo && productInfo.params?.images?.length > 1 && (
